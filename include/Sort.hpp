...
namespace Sorters {
    ...
    template <typename T>
    int toBotton(const T& a, const T& b) {
        if (a > b) return -1;
        if (b > a) return 1;
        return 0;
    }
    ...
    template <typename T>
    bool isSort(const List<T>& list, int (*cmp)(const T&, const T&)) {
        int n = list.last();
        if (n <= 0) return true;
    ...
    template <typename T>
    void bubbleSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        int n = list.last();
        if (n <= 0) return;
    ...
    template <typename T>
    void insertSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        int n = list.last();
        for (int i = 1; i <= n; ++i) {
            T key = list.get(i);
            int j = i - 1;
    ...
    template <typename T>
    void selectSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        int n = list.last();
        for (int i = 0; i < n; ++i) {
            int minIndex = i;
            for (int j = i + 1; j <= n; ++j) {
    ...
    template <typename T>
    void shellSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        int n = list.last() + 1;
    ...
    template <typename T>
    void merge(List<T>& list, int left, int mid, int right, int (*cmp)(const T&, const T&)) {
        int n1 = mid - left + 1;
        int n2 = right - mid;
    ...
    template <typename T>
    void mergeSortHelper(List<T>& list, int left, int right, int (*cmp)(const T&, const T&)) {
        if (left < right) {
    ...
    template <typename T>
    void mergeSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        if (list.last() > 0)
            mergeSortHelper(list, 0, list.last(), cmp);
    }

    template <typename T>
    int partition(List<T>& list, int low, int high, int (*cmp)(const T&, const T&)) {
    ...
    template <typename T>
    void quickSortHelper(List<T>& list, int low, int high, int (*cmp)(const T&, const T&)) {
        if (low < high) {
    ...
    template <typename T>
    void quickSort(List<T>& list, int (*cmp)(const T&, const T&)) {
        if (list.last() > 0)
